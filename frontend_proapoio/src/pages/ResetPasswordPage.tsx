...
      announce('Erros no formulário. Por favor, corrija os campos destacados.');
      return;
    }
...
        msg = err?.response?.data?.message || 'Token inválido, expirado ou senha não atende aos requisitos.';
      } else if (status === 429) {
        msg = 'Muitas tentativas. Aguarde um minuto e tente novamente.';
      } else {
        msg = 'Não foi possível redefinir a senha devido a um erro de rede.';
      }
      
      setError(msg);
      announce(msg);
