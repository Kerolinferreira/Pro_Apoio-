...
// Assumindo que você usa componentes de UI simples
const LoadingSpinner = () => <div className="text-center py-10">Carregando...</div>;
const ErrorAlert = ({ message }) => <div className="text-center py-10 text-red-600">{message}</div>;
const Button = ({ children, onClick, disabled = false }) => (
    <button onClick={onClick} disabled={disabled} className={`px-6 py-3 rounded-lg font-semibold text-white transition-colors duration-200 ${disabled ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700'}`}>
        {children}
    </button>
);


// 1. ATUALIZAÇÃO DA INTERFACE VAGA
...
interface Vaga {
    ...
    salario: number | null;
    localizacao: string;
    // Novos campos cruciais, conforme a documentação
    deficiencias: Deficiencia[]; // Lista de deficiências associadas (e.g., Visual, Auditiva)
    necessidades_descricao: string; // Descrição detalhada das necessidades/tarefas
    instituicao?: {
        id: number;
...
const DetalhesVagaPage: React.FC = () => {
    ...
    // Assumindo que o usuário logado é um Candidato para poder "Candidatar-se"
    const isCandidato = true; 
    const [isSaved, setIsSaved] = useState(false); // Estado para Vaga Salva
    const [isApplied, setIsApplied] = useState(false); // Estado para Proposta Enviada
    ...
    useEffect(() => {
        const fetchVaga = async () => {
            ...
            try {
                ...
                // Checagens de estado (Se a vaga foi salva ou se o candidato já se aplicou)
    ...
    const handleApply = () => {
        // Lógica para enviar proposta POST /propostas
        alert('Implementar envio de proposta');
        setIsApplied(true);
    };

    const handleSave = () => {
        // Lógica para salvar/desalvar vaga POST/DELETE /candidatos/me/vagas-salvas
    ...
    if (!vaga) {
        return <ErrorAlert message="Vaga não encontrada." />;
    }

    const formatSalary = (salary: number | null) => {
        return salary ? `R$ ${salary.toFixed(2).replace('.', ',')}` : 'A combinar';
    };
                ...
                <div className="bg-white shadow-xl rounded-lg p-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-8 text-gray-700">
                        ...
                        <div>
                            ...
                            <p className="text-lg font-semibold">{new Date(vaga.data_publicacao).toLocaleDateString()}</p>
                        </div>
                    </div>

                    {/* 2. INÍCIO DA CORREÇÃO: Informações Cruciais (Deficiências e Necessidades) */}
                    <section className="mt-8 pt-6 border-t border-gray-100">
                        <h2 className="text-2xl font-bold text-gray-800 mb-4">Requisitos Específicos do Aluno</h2>
                        
                        {/* Deficiências / Condição do Aluno */}
                        <div className="mb-6">
                            <h3 className="text-lg font-semibold text-gray-700 mb-2">Deficiências Associadas</h3>
                            {vaga.deficiencias && vaga.deficiencias.length > 0 ? (
                                <div className="flex flex-wrap gap-2">
                                    {vaga.deficiencias.map((def) => (
                                        <span key={def.id} className="px-3 py-1 bg-purple-100 text-purple-800 text-sm font-medium rounded-full">
                                            {def.nome}
                                        </span>
                                    ))}
                                </div>
                        ...
                        {/* Necessidades / Descrição do Apoio */}
                        <div>
                            <h3 className="text-lg font-semibold text-gray-700 mb-2">Descrição Detalhada das Necessidades (Tarefas/Apoio)</h3>
                    ...
                    {isCandidato && (
                        <div className="mt-8 pt-6 border-t border-gray-100 flex justify-end space-x-4">
                            <Button 
                                onClick={handleSave} 
                                disabled={isApplied} 
                                variant={isSaved ? 'secondary' : 'primary-outline'} // Simulação de um botão secundário
                                className={`bg-white text-blue-600 border border-blue-600 hover:bg-blue-50 ${isApplied ? 'hidden' : ''}`}
                            >
