...
export default function VagasSalvasPage() {
  ...
  return (
    <main className="p-4 max-w-5xl mx-auto" aria-labelledby="titulo-vagas-salvas">
      ...
      <h1 id="titulo-vagas-salvas" className="text-2xl font-extrabold mb-4">Vagas salvas</h1>
      ...
      {!loading && !error && vagas.length === 0 && (
        <div className="rounded border p-6 text-center">
          ...
          <p className="text-sm text-zinc-600 mt-1">Veja <Link to="/vagas" className="underline">vagas disponíveis</Link> e salve as que interessarem.</p>
        </div>
      )}
      ...
      {!loading && !error && vagas.length > 0 && (
        <ul className="space-y-2" role="list">
          {vagas.map((v) => (
            <li key={v.id} className="border p-3 rounded flex justify-between items-start gap-3 bg-white shadow-sm">
              <div>
                <h2 className="font-semibold text-zinc-900">{v.vaga.titulo_vaga}</h2>
                <p className="text-sm text-zinc-700">{v.vaga.instituicao?.nome_fantasia || ''} • {v.vaga.cidade}</p>
                <p className="text-sm text-zinc-700">Regime: {v.vaga.regime_contratacao}</p>
              </div>
              <div className="flex items-center gap-2">
                <Link to={`/vagas/${v.vaga.id}`} className="rounded bg-zinc-200 px-3 py-1.5 text-sm">Ver detalhes</Link>
                <button
      ...
      {lastRemoved && (
        <div className="mt-3 rounded border border-amber-200 bg-amber-50 p-2 text-amber-900 text-sm flex items-center justify-between" role="status">
          <span>Vaga removida. </span>
          <button onClick={desfazer} className="underline">Desfazer</button>
        </div>
      )}
    </main>
  )
