...
export default function Header() {
  // OBSERVAÇÃO: A linha 'import React from "react";' deve ser REMOVIDA se estiver aqui!
  ...
  const navLinks = React.useMemo(() => {
    ...
    if (user.tipo_usuario === 'instituicao') {
      return [
        { to: "/buscar-candidatos", label: "Buscar Agentes", icon: <Search size={18} /> },
      ...
      return [
        { to: "/buscar-vagas", label: "Buscar Vagas", icon: <Search size={18} /> },
    ...
    <header className="header-logged" role="banner">
      <div className="container header-container">
        ...
        <nav aria-label="Navegação do usuário" className="main-nav-logged">
            <ul className="flex-group-item" role="list" style={{gap: 'var(--spacing-md)'}}>
                {navLinks.map(link => (
                    <li key={link.to}>
        ...
        <div className="flex-group-item" style={{gap: 'var(--spacing-md)'}}>
